...
import (
	...
	dpfm_api_output_formatter "convert-to-dpfm-orders-from-sap-sales-order/DPFM_API_Output_Formatter"
	dpfm_api_processing_data_formatter "convert-to-dpfm-orders-from-sap-sales-order/DPFM_API_Processing_Formatter"
	"sync"
...
func (c *ConvertComplementer) CreateSdc(
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		// 1-0. データ連携基盤Orders HeaderとSAP Sales Orderとの項目マッピング変換
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		// 5-1. データ連携基盤Orders ItemとSAP Sales Orderとの項目マッピング変換
	...
	go func(wg *sync.WaitGroup) {
		...
		// Item
		// <5-3. 変換元のCustomer(DeliverToParty)のセット>
		psdc.SetCustomerItem = c.SetCustomerItem(sdc, psdc)
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		// 8-1. データ連携基盤Orders Item Pricing ElementとSAP Sales Orderとの項目マッピング変換
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		// 4-1. データ連携基盤Orders Item Schedule LineとSAP Sales Orderとの項目マッピング変換
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		// 6-1. データ連携基盤Orders AddressとSAP Sales Orderとの項目マッピング変換
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		// 2-1. データ連携基盤Orders PartnerとSAP Sales Order との項目マッピング変換
